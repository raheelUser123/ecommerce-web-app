...
export default async function EditProductPage({
  ...
  const [{ data: product }, { data: categories }, { data: productCategory }] =
    await Promise.all([
      ...
      supabaseAdmin.from("product_categories").select("*").eq("product_id", params.id).maybeSingle(),
    ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={updateProduct} className="space-y-6">
        ...
        <div className="card space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="title" defaultValue={product.title} required className="input mt-1" />
            </div>
          ...
          <div>
            ...
            <textarea name="description" rows={5} defaultValue={product.description || ""} className="input mt-1" />
          </div>

          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-bold uppercase">Base Price</label>
              <input name="price" type="number" step="0.01" defaultValue={product.price || 0} className="input mt-1" />
            </div>

            <div>
              <label className="text-xs font-bold uppercase">Category</label>
              <select name="category_id" defaultValue={productCategory?.category_id || ""} className="input mt-1">
            ...
            <div>
              ...
              <select name="status" defaultValue={product.status || "active"} className="input mt-1">
        ...
        <div className="card space-y-4">
          <h2 className="text-xl font-bold text-dark">Image</h2>

          {product.main_image && (
            <img src={product.main_image} className="w-32 h-32 object-cover rounded-xl border" />
          )}
